...
def max((a_start,a_end), (b_start,b_end)):
    if a_end > b_end:
...
def createContext(host, port, outputPath1, outputPath2):
    ...
    tripsByCellsAndWindows = queryOneFormat.map(lambda trip: ((trip.start_cell_id, trip.end_cell_id), (trip.pickup_datetime, trip.dropoff_datetime))).window(30,10).cache()
...
#    tripsByCellsAndWindows.checkpoint(120)
    ...
    tripMaxDropoffBycell = tripsByCellsAndWindows.transform(lambda rdd: rdd.reduceByKey(lambda a,b: max(a,b)).map(lambda (k,v): (k,("%s"%v[0],"%s"%v[1]))))
...
#    topThreeWordCountsWithIndex = sortedWordCounts.transform(lambda rdd: rdd.zipWithIndex().filter(lambda (k,v): v<3).map(lambda (k,v):(v,k)))
    ...
    def updateFunc(new_values, last_list):
        ...
        new_ranking = [k for (k,v) in new_values]
        ...
        if len(diff)>0:
            ...
            new_dates = max([v for (k,v) in new_values if k in diff])
...
#u    rankingTripCount.pprint()
    #invoke updateFunc for each time window
#i    status = rankingTripCount.updateStateByKey(updateFunc)
#i    status.pprint()
