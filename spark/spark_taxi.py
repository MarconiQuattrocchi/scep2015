...
def createContext(host, port, outputPath1, outputPath2):
    ...
    tripsByCellsAndWindowsForCount = queryFormat.map(lambda trip: ((trip.start_cell_id, trip.end_cell_id), (trip.pickup_datetime, trip.dropoff_datetime))).window(10,1).cache()
...
#    tripsByCellsAndWindows.checkpoint(120)
    ...
    ####QUERY2
    tripsByCellsAndWindowsForProfit = queryFormat.map(lambda trip: (trip.start_cell_id, trip.fare_amount + trip.tip_amount)).window(5,3).cache()

    tripsByMedallionsAndWindowsForEmptyTaxies = queryFormat.map(lambda trip: (trip.medallion, (trip.end_cell_id, trip.dropoff_datetime))).window(5,3).cache()

    profitByCell = tripsByCellsAndWindowsForProfit.transform(lambda rdd: rdd.groupByKey().map(lambda (k,v): (k,median(v))).sortByKey(False))
...
#   lastTripsByMedallion.pprint()
    countEmptyTaxiesByCells = lastTripsByMedallion.transform(lambda rdd: rdd.map(lambda (k,v):(v[0],1)).reduceByKey(lambda x,y:x+y).sortBy(lambda (k,v):v, ascending=False))
#    countEmptyTaxiesByCells.pprint()

    profitabilityByCell = profitByCell.join(countEmptyTaxiesByCells).map(lambda x:('CULO',x))
    profitabilityByCell.pprint()
